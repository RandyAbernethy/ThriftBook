...
var wsFrame = {
   ...
   *
   * data - data.length is the assumed payload size
   * maks - true if a mask will be sent (TO_SERVER)
   */
...
// createThriftWebServer constructor and options
/////////////////////////////////////////////////////////////////////
 ...
 * @param {ThriftWebServerOptions} - The server configuration.
...
exports.createStaticHttpThriftServer = function(options) {
  ...
  function processWS(data, socket) {
    ...
    svc.transport.receiver(function(transportWithData) {
      ...
      var output = new svc.protocol(new svc.transport(undefined, function(buf) {
        try {
          var textPayload = new Buffer('[1,"get_message",2,0,{"0":{"str":"3) Hello Fred!"}}]', 'utf8');
  ...
  //Wire up listeners for GET(files), POST(XHR), upgrade(WebSocket)
  server.on('request', function(request, response) {
    ...
    socket.on('data', function(frame) {
      ...
      console.log("Inbound frame:");
      console.log(wsFrame.decode(frame).data.toString('utf8'));
      processWS(wsFrame.decode(frame).data, socket);
    });
  });
